...
//Akın Kaldıroğlu udemy eğitiminden alınmıştır
...
//		listFromArray();
    ...
    public static void processList() {
        ...
        System.out.println("Index of 3: " + list.indexOf(3)); //verilen elemanın ilk indexini dönüyor
        System.out.println("last index of 3: " + list.lastIndexOf(3)); //verilen elemanın son görüldüğü yeri dönüyor

        System.out.println();

        System.out.println("Set 1st element: " + list.set(1, "Mustafa")); //1. indexdeki elemanı Mustafaya set ediyor
        System.out.println("Akin is in the list? " + list.contains("Akin"));
        ...
        stringList.sort(stringComparator);//Nasıl sıralanacağına dair dışardan comparator verdik
        stringList.forEach(System.out::println);

        System.out.println("\nProcessing a string list with a unary operation");
        UnaryOperator<String> operator = s -> s.toUpperCase();
        stringList.replaceAll(operator); //lambda ifadesi ile listteki tüm stringleri upper yaptık
        stringList.forEach(System.out::println);

        System.out.println("\nProcessing a string list with a predicate");
        Predicate<String> stringLengthPredicate = s -> s.length() > 3;
        stringList.removeIf(stringLengthPredicate); //verilen predicate'i sağlayan elemanları remove ediyor
        stringList.forEach(System.out::println);
    }
...
//		list.add("Last List"); // Run time error: java.lang.UnsupportedOperationException //asList() size'ı sabit list döner

        list.set(0, "Black List"); //dikkat bunu değiştirice array'deki elemanda değişti birbirini mirror ediyorlar gibi düşün
        System.out.println("\nList");
        list.forEach(System.out::println);
